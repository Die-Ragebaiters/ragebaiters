...
create table if not exists public.doodle_jason_score_entries (
  id bigint generated by default as identity primary key,
  user_id uuid not null references auth.users (id) on delete cascade,
  score integer not null check (score >= 0),
  created_at timestamptz not null default now(),
  constraint doodle_jason_score_entries_user_score_unique unique (user_id, score)
);

revoke all on public.doodle_jason_scores from public, anon, authenticated;
revoke all on public.doodle_jason_score_entries from public, anon, authenticated;

create or replace function public.block_doodle_jason_score_entry_mutation()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  raise exception 'Doodle-Jason-Highscores koennen nachtraeglich nicht mehr veraendert oder geloescht werden.';
end;
$$;

drop trigger if exists doodle_jason_score_entries_immutable on public.doodle_jason_score_entries;
create trigger doodle_jason_score_entries_immutable
before update or delete on public.doodle_jason_score_entries
for each row
execute function public.block_doodle_jason_score_entry_mutation();

insert into public.doodle_jason_score_entries (user_id, score, created_at)
select
  user_id,
  best_score,
...
where best_score > 0
on conflict (user_id, score) do nothing;
...
as $$
  select coalesce(
    (select max(score) from public.doodle_jason_score_entries where user_id = auth.uid()),
    0
  );
$$;
...
as $$
  with best_per_user as (
    ...
    from public.doodle_jason_score_entries
    group by user_id
  )
  select
    coalesce(p.username, 'mitglied') as username,
    b.best_score,
    b.user_id = auth.uid() as is_current_user
  from best_per_user b
  left join public.profiles p on p.id = b.user_id
  where b.best_score > 0
  order by b.best_score desc, coalesce(lower(p.username), 'mitglied') asc
  limit 10;
$$;
...
begin
  ...
  if v_score > v_best then
    insert into public.doodle_jason_score_entries (user_id, score)
    values (auth.uid(), v_score)
    on conflict (user_id, score) do nothing;
...
-- 2026-05-06: Generisches Secret-Arcade-Highscore-System
create table if not exists public.arcade_game_score_entries (
  id bigint generated by default as identity primary key,
  ...
  constraint arcade_game_score_entries_game_key_check
    check (game_key in ('doodle_jason', 'space_invaders', 'superjason', 'monkeykong', 'oblock')),
  constraint arcade_game_score_entries_unique unique (game_key, user_id, score)
);

alter table public.arcade_game_score_entries
  drop constraint if exists arcade_game_score_entries_game_key_check;
...
create or replace function public.block_arcade_game_score_entry_mutation()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  raise exception 'Arcade-Highscores koennen nachtraeglich nicht mehr veraendert oder geloescht werden.';
end;
$$;

drop trigger if exists arcade_game_score_entries_immutable on public.arcade_game_score_entries;
create trigger arcade_game_score_entries_immutable
before update or delete on public.arcade_game_score_entries
for each row
execute function public.block_arcade_game_score_entry_mutation();

insert into public.arcade_game_score_entries (game_key, user_id, score, created_at)
select
  'doodle_jason',
  user_id,
  score,
  created_at
from public.doodle_jason_score_entries
on conflict (game_key, user_id, score) do nothing;

create or replace function public.get_arcade_my_highscore(p_game_key text)
returns integer
language sql
security definer
set search_path = public
...
as $$
  select coalesce(
    (
      select max(score)
      from public.arcade_game_score_entries
      where game_key = trim(lower(coalesce(p_game_key, '')))
...
create or replace function public.get_arcade_leaderboard(p_game_key text)
returns table (
  username text,
  best_score integer,
  is_current_user boolean
...
create or replace function public.submit_arcade_score(p_game_key text, p_score integer)
returns integer
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if v_game_key not in ('doodle_jason', 'space_invaders', 'superjason', 'monkeykong', 'oblock') then
  ...
  if v_score > v_best then
    insert into public.arcade_game_score_entries (game_key, user_id, score)
    values (v_game_key, auth.uid(), v_score)
    on conflict (game_key, user_id, score) do nothing;
...
grant execute on function public.get_arcade_my_highscore(text) to authenticated;
grant execute on function public.get_arcade_leaderboard(text) to authenticated;
grant execute on function public.submit_arcade_score(text, integer) to authenticated;
